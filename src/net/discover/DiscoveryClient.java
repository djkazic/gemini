			...
			try {
				...
				try {
					DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length,
							InetAddress.getByName("255.255.255.255"), Core.config.discoverPort);
				...
				if (message.equals("DISC_RAD_RESP")
						&& !NetHandler.foundHosts
								.contains(InetAddress.getByName(receivePacket.getAddress().getHostAddress()))
					...
					try {
						...
						// Utilities.log(this, "Local peer identified: " +
						// potentialPeer);
					} catch (Exception ex) {
						ex.printStackTrace();
					}
